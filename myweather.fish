function myweather
    ...
    if test -z $LOC_ID
        ...
        echo "  curl -s -L 'https://api.data.gov.my/weather/forecast' | jq '[.[].location | {\"key\": .location_id, \"value\": .location_name}] | from_entries'"
        return 1
    end
    ...
    if not test -e $CACHE_FILE || test (math (date -d 0 +%s) - (path mtime $CACHE_FILE)) -ge 0
        curl -s -L "https://api.data.gov.my/weather/forecast?contains=$LOC_ID@location__location_id" >$CACHE_FILE
    end
